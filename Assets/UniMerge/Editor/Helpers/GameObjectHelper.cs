...
namespace UniMerge.Editor.Helpers {
	public class GameObjectHelper : Helper {
		...
		bool sameAttrs, sameCompoenents, showComponents, showAttrs;
				...
				if (i == 0) {
					if (children != null) {
						...
						children.Sort(delegate (GameObjectHelper a, GameObjectHelper b) {
							...
							var bMine = b.mine;
							if (aMine && bMine)
								return aMine.name.CompareTo(bMine.name);
							var bTheirs = b.theirs;
							if (aMine && bTheirs)
								return aMine.name.CompareTo(bTheirs.name);
							var aTheirs = a.theirs;
							if (aTheirs && bMine)
								return aTheirs.name.CompareTo(bMine.name);
							if (aTheirs && bTheirs)
								return aTheirs.name.CompareTo(bTheirs.name);
							return 0;
						});
					}
					...
					helper.sameAttrs = true;
					count = attributes.Count;
					// ReSharper disable once ForCanBeConvertedToForeach
					for (var j = 0; j < count; j++) {
						var attribute = attributes[j];
						if (!attribute.Same)
							helper.sameAttrs = false;
					}

					if (!helper.sameAttrs && window.compareAttrs)
						helper.Same = false;
				}
		...
		public void Draw(float colWidth, float indent =  4) {
			DeepDraw(this, window, GUILayout.Width(colWidth + UniMergeWindow.columnPadding), indent);
		}
					...
					if (draw) {
						...
						if (foldoutState != showChildren) {
							...
							if (Event.current.alt)
								helper.SetFoldoutRecursively(showChildren);
						}
						...
						if (GUILayout.Button(showComponents ? "-" : "+", Width19)) {
							...
							if (Event.current.alt) {
								helper.showAttrs = showComponents;
								foreach (var component in components)
									component.SetFoldoutRecursively(showComponents);
							}
						}
						GUILayout.EndHorizontal();
						GUILayout.EndVertical();

						window.EndRow(helper.sameAttrs && helper.sameCompoenents);
					}
		...
		void SetFoldoutRecursively(bool state) {
			showChildren = state;
			if (children != null)
				foreach (var obj in children)
					obj.SetFoldoutRecursively(state);
		}
